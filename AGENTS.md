...
- Micro Chat "新的好友" should open its own WeChat-like page showing current and historical add requests. Moments should use a top background image, the user's own avatar in the middle/right area, and blank white content below for future generated posts.
- Micro Chat contacts, moments, and settings tabs should not show the app-level back button. Moments should hide the inner four-tab bar, use a full-phone background image, and show a top-right action symbol that opens generation controls for random/specified mode and post count.
