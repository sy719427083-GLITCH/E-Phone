...
- Latest lock-screen wallpaper direction is a pure-white fairy-tale city road with clean manga/comic feeling, low-saturation pink accents, and a small cat plus dog together near the bottom; avoid noisy texture and avoid green as the main color. Latest home wallpaper direction is pure white with manga/comic freshness, a cat in the upper windowsill area, curtains, visible pale green plants, and an airy icon-friendly middle/lower area.
...
- The role and identity apps now follow the imported "Characters & Me - Independent Core" direction from `DOCTYPE html.docx`: role overview uses a monochrome high-fashion magazine archive layout with main/NPC/relation tabs, while identity overview uses a Morandi gallery/persona layout. Keep the existing React data, save/edit/delete, avatar upload, API generation, and world-book binding behavior behind that visual shell.
